...
@st.cache
def load_data():
    read_data = pd.read_csv("C:/Users/mi/ml/notebooks/KR/data/diamonds.csv", sep=',')
    del read_data['Unnamed: 0']
    return read_data


st.header('Dataset')
read_state = st.text('Reading dataset...')
data = load_data().copy()
read_state.text('Analyze diamonds by their cut, color, clarity, price, and other attributes')
st.subheader('Diamonds:')
st.write(data.head())

regr_models = []

if st.sidebar.checkbox('Limit rows'):
...
if st.sidebar.checkbox('KNeighborsRegression'):
    ...
    if st.sidebar.checkbox("KNeighborsRegression(Best)"):
        n_neighbors_search_1 = st.sidebar.slider("starts from", 1, 100)
        n_neighbors_search_2 = st.sidebar.slider("ends with", 1, 100, value=10)
        n_neighbors_search_step = st.sidebar.slider("step", 1, 100, 1)
...
if st.sidebar.checkbox('GradientBoostingRegressor'):
    ...
    if st.sidebar.checkbox('GradientBoostingRegressor(Best)'):
        n_estimators_search_1 = st.sidebar.slider("starts from", 100, 1000, value=100)
        n_estimators_search_2 = st.sidebar.slider("ends with", 1, 1000, value=101)
        n_estimators_search_step = st.sidebar.slider("step", 100, 1000, value=100)
